...
int SCR_WIDTH  =   -1   ;           // 窗口宽度
int SCR_HEIGHT =   -1   ;           // 窗口高度
#define MAX_BALL       501          // 球数最大值
const float ball_op_val=0.0001f;    // 加速参数
const float sp_go_down=0.1f;        // 减速参数
const float MAX_SP=100.2f;          // 咱的最大速度
...
inline void player_ctrl(bool _using=true) {
    ...
    while(is_run()) {
        ...
        if(switch_mouse_left) {
            ...
            float ddx=keydown['D']-keydown['A'],ddy=keydown['S']-keydown['W']+.4; // 加重力
...
int main() {
    ...
    if(SCR_WIDTH==-1||SCR_HEIGHT==-1) {
        getviewport(&SCR_WIDTH,&SCR_HEIGHT,&SCR_WIDTH,&SCR_HEIGHT);
        // cout<<SCR_WIDTH<<endl;
